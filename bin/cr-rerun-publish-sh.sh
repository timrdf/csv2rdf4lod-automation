...
if [ $# -lt 1 ]; then
   echo "usage: `basename $0` [-n] [-con {raw,e1}] [[cr:ALL] | datasetIdentifier [datasetIdentifier] ...]"
   echo ""
   ...
   for versionDir in $versionDirs; do
      ...
      if [ ${#versionDir} -gt 0 ]; then
         ...
         pushd $versionDir &> /dev/null
            publish_script=publish/bin/publish.sh
            if [ -e $publish_script ]; then
               for conversionIdentifier in `echo $conversionIdentifiers`; do
                  ...
                  #fi
                  if [ ${#focusConversion} -eq 0 ]; then
                     # No focus conversion was specified, so process all of them.
                     echo "`basename $0` republishing conversion: $conversionIdentifier ($publish_script)"
                     if [ ${dryRun:-"."} != "true" ]; then
                        $publish_script
                     fi
