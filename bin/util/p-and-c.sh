...
while [ $# -gt 0 ]; do
   ...
   if [[ -n "$as_ttl" ]]; then
      ...
      if [[ $total -eq 1 && `gzipped.sh $file` == "yes" && `guess-syntax.sh $file mime` == "text/plain" ]]; then
         ...
         gunzip -c             $file | awk '{if($2 == "<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>"){ print $3 }else{ print $2 }}'
      else
         #echo ".${total}. .`gzipped.sh $file`. .`guess-syntax.sh $file mime`." >&2
         # Handles any syntax, compressed or not.
         rdf2nt.sh --version 2 $file | awk '{if($2 == "<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>"){ print $3 }else{ print $2 }}'
      fi
   fi
