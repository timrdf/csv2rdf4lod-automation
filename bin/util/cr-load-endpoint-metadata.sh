...
pushd ${CSV2RDF4LOD_PUBLISH_LOD_MATERIALIZATION_WWW_ROOT}/query 
   ...
   #
   $sudo cp $CSV2RDF4LOD_HOME/bin/util/cr-load-endpoint-metadataset.trq cr-load-endpoint-metadataset.rq
   $sudo perl -pie "s|?:graph|<$datasetGraph>|" cr-load-endpoint-metadataset.rq
   echo
