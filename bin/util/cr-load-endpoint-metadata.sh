...
pushd ${CSV2RDF4LOD_PUBLISH_LOD_MATERIALIZATION_WWW_ROOT}/query 
   ...
   #
   #$sudo cat $CSV2RDF4LOD_HOME/bin/util/cr-load-endpoint-metadataset.trq | perl -pe "s|\?:graph|<$datasetGraph>|" > cr-load-endpoint-metadataset.rq
   $sudo cat $CSV2RDF4LOD_HOME/bin/util/cr-load-endpoint-metadataset.trq | sed "s/\?:graph/<`cat $datasetGraph | sed 's/\//\\\//'`>/" > cr-load-endpoint-metadataset.rq
   echo
