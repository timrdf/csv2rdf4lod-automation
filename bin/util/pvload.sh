...
escapedEndpoint=`echo "http://logd.tw.rpi.edu:8890/sparql" | perl -e 'use URI::Escape; @userinput = <STDIN>; foreach (@userinput) { chomp($_); print uri_escape($_); }'`
...
while [ $# -gt 0 ]; do
   ...
   if [ `wc -l ${TEMP}${unzipped}.nt | awk '{print $1}'` -gt 0 ]; then
      ...
      # TODO: rm 8890 reference:
      HACK="http%3A%2F%2Flogd.tw.rpi.edu%3A8890%2Fsparql"
      echo CHECK: $HACK
      echo CHECK: $escapedEndpoint
