...
while [ $# -gt 0 ]; do
   ...
   if [ `wc -l ${TEMP}${unzipped}.nt | awk '{print $1}'` -gt 0 ]; then
      ...
      #latest_NG_nodeset=`$java_saxon endpoint=${CSV2RDF4LOD_PUBLISH_VIRTUOSO_SPARQL_ENDPOINT} named-graph=${named_graph}` # pvload-latest-ng-load.xsl can call vsr:endpoint for a generic call.
